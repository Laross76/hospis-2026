...
public class NewsPage {
    ...
    public void verifySortButtonClickable() {
        Allure.step("Проверка кликабельности кнопки сортировки");
        SORT_NEWS_MATERIAL.check(matches(allOf(isDisplayed(), isClickable())));
    }

    public void verifyFilterButtonClickable() {
        Allure.step("Проверка кликабельности кнопки фильтра");
        FILTER_NEWS_MATERIAL.check(matches(allOf(isDisplayed(), isClickable())));
    }
    ...
    private void scrollToItem(int position) {
        ...
        onView(withId(NEWS_LIST_RECYCLER_VIEW)).perform(scrollToPosition(position));
    }
    ...
    public void checkAllNewsStatus(String expectedStatus) {
        ...
        for (int i = 0; i < getNewsItemCount(); i++) {
            ...
            assertEquals("Ожидается статус '" + expectedStatus + "' для элемента " + i, expectedStatus, actualStatus);
        }
    }
    ...
    public void checkAllNewsDateRange(int fromWhatDay, int untilWhatDay) {
        ...
        for (int i = 0; i < getNewsItemCount(); i++) {
            ...
            assertTrue("Ожидается, что дата для элемента " + i + " будет в диапазоне от " + startDate + " до " + endDate,
                    (actualDate.isEqual(startDate) || actualDate.isEqual(endDate) || (actualDate.isAfter(startDate) && actualDate.isBefore(endDate))));
        }
    }
    public void verifyTextAfterCancelingFilter(String text) {
        ...
        ViewInteraction textView = onView(allOf(withText(text), withParent(withParent(IsInstanceOf.<View>instanceOf(android.widget.LinearLayout.class))), isDisplayed()));
